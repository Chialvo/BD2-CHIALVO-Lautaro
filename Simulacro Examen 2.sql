...
(SELECT COUNT(f.NRO_FACTURA) FROM FACTURAS f WHERE f.COD_CLIENTE = c.COD_CLIENTE) AS 'Cantidad de compras',
(SELECT SUM(p.PRECIO * df.CANTIDAD)FROM DETALLES_FACTURAS df INNER JOIN FACTURAS f USING (NRO_FACTURA) INNER JOIN PLANTAS p USING (COD_PLANTA) WHERE f.COD_CLIENTE = c.COD_CLIENTE) AS 'Total Gastado',
